window.addEventListener('DOMContentLoaded', async function() {
  ...
  document.querySelectorAll('.repo-card').forEach(async function(el) {
    ...
    data.description = el.getAttribute('data-alt');

    if (!(data.hasOwnProperty('forks'))) {
      data.html_url = 'https://github.com/' + name;
      data.name = name.split('/')[1];
      data.fork = false;
      data.source = {};
      data.source.html_url = 'https://github.com/' + name;
      data.source.full_name = name;
    ...
    el.innerHTML = `
    <div class="shadow-sm" style="font-family: -apple-system,BlinkMacSystemFont,Segoe UI,Helvetica,Arial,sans-serif,Apple Color Emoji,Segoe UI Emoji; background: white; padding: 16px; font-size: 14px; line-height: 1.5; color: #24292e; height:100%; width:100%; min-height: 150px;"> 
      ...
      <div style="font-size: 12px; font-weight: 400; margin-bottom: 16px; margin-top: 8px; color: #586069;">${data.description}</div>
      <div style="font-size: 12px; color: #586069; display: flex; position: absolute; bottom: 16px;">
        <div style="${data.language ? '' : 'display: none'}; margin-right: 16px;">
          <span style="width: 12px; height: 12px; border-radius: 100%; background-color: ${data.language ? colors[data.language].color : ''}; display: inline-block; top: 1px;"></span>
